...
// minkowsky with p=2.0 in 2D is euclidean
inline float distance(Vector2 a, Vector2 b, double p)
{
    // return std::sqrt(sqr_distance(a, b)); // euclidean
    return std::pow(std::pow(std::abs(a.x - b.x), p) + std::pow(std::abs(a.y - b.y), p), 1 / p); // minkowsky
}
...
Grid<int> voronoi(int width, int height, int numPoints, double exponent, unsigned int seed)
{
    ...
    for (int i = 0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            currPoint.x = i;
            currPoint.y = j;
            dist = distance(nearestNeighbor(currPoint, points, numPoints), currPoint, exponent);
            pixels[i][j] = dist;
